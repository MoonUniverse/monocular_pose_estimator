...
public:
   ...
   */
  int DetectLandmarks(const cv::Mat &img,
  ...
  std::vector<cv::Point>
      clusteredPixels_; /**< Keeps a copy of pixel clusters found */
  std::vector<Cluster>
      clusteredPoints_; /**< Keeps a copy of point clusters found*/

  uint8_t threshold; /**< Threshold for grayvalue thresholding 0-254*/
  uint32_t tight_filter_size;
  uint32_t wide_filter_size;
  float maxRadiusForPixelCluster; /**< Maximum radius for clustering pixels to
                                     marker points*/
  ...
  float maxRadiusForCluster; /**< Maximum radius for clustering marker points to
                                landmarks*/
  uint16_t minPointsPerLandmark;    /**< Minimum count of marker points per
                                       landmark (0)*/
  uint16_t maxPointsPerLandmark;    /**< Maximum count of marker points per
                                       landmark (depends on grid used)*/
...
private:
  ...
  /**
   * @brief Finds hypotheses for marker points by thresholding the input image
   * and clustering the pixels.
   *
   * @param img_in
  ...
  void FindClusters(const std::vector<cv::Point> &points_in,
                    std::vector<Cluster> &clusters, const float radiusThreshold,
                    const unsigned int minPointsThreshold,
                    const unsigned int maxPointsThreshold);
  /**
   * @brief Identifies the three corner points of a landmark and moves them into
   * the second vector. It utilizes a score function to find the triple.
   *
   ...
   */
  bool FindCorners(std::vector<cv::Point> &point_list,
  ...
  FindLandmarks(const std::vector<Cluster> &clusteredPoints);
  /**
   ...
   * @return bool Success
   */
  bool CalculateIdForward(ImgLandmark &landmark,
   ...
   * @param points
   */
  void parallel_vector_sort(std::vector<uint16_t> &ids,
