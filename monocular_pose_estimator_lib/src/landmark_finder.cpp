...
//
// Copyright 2016 Claudio Bandera <claudio.bandera@kit.edu (Karlsruhe Institute
// of Technology)
//
...
///--------------------------------------------------------------------------------------///
int LandmarkFinder::DetectLandmarks(
    const cv::Mat &img, std::vector<ImgLandmark> &detected_landmarks) {
  ...
  FindClusters(clusteredPixels_, clusteredPoints_, maxRadiusForCluster,
               minPointsPerLandmark, maxPointsPerLandmark);
...
void LandmarkFinder::FilterImage(const cv::Mat &img_in, cv::Mat &img_out) {
  ...
  if (tight_filter_size == 0) {
    ...
    cv::boxFilter(img_in, tight_filtered, -1,
                  cv::Size(tight_filter_size, tight_filter_size),
                  cv::Point(-1, -1), true, cv::BORDER_DEFAULT);
  }
  cv::boxFilter(img_in, wide_filtered, -1,
                cv::Size(wide_filter_size, wide_filter_size), cv::Point(-1, -1),
...
std::vector<cv::Point> LandmarkFinder::FindPoints(cv::Mat &img_in) {
  ...
  FindClusters(pixels, clusteredPixels, maxRadiusForPixelCluster,
               minPixelForCluster, maxPixelForCluster);
...
void LandmarkFinder::FindClusters(const std::vector<cv::Point> &points_in,
  ...
  for (auto &thisPoint : points_in) /// go thru all points
  {
    ...
    for (auto &cluster :
         boost::adaptors::reverse(clusters)) { /// go thru all clusters
      for (auto &clusterPoint :
           cluster) { /// go thru all points in this cluster
      ...
      if (clusterFound) /// because point has been added to cluster, no further
...
///--------------------------------------------------------------------------------------///
/// FindCorners identifies the three corner points and sorts them into output
/// vector
/// -> find three points whos sum of length is maximum and two edges are
/// perpendicular
///--------------------------------------------------------------------------------------///
bool LandmarkFinder::FindCorners(std::vector<cv::Point> &point_list,
  ...
  /*  Numbering of corners and coordinate frame FOR THIS FUNCTION ONLY // TODO
  ...
  for (size_t i = 0; i < point_list.size(); i++) {
    ...
    for (size_t j = i + 1; j < point_list.size(); j++) {
      ...
      for (size_t k = j + 1; k < point_list.size(); k++) {
        ...
        /// Since we test every combination only once, make sure the lengths are
        /// correct:
        // norm(v12) > norm(v31) >= (v32)
        if ((cv::norm(v31) > fp * cv::norm(v32)) &&
            (cv::norm(v31) > fp * cv::norm(v12))) {
          v12 = v31; // v12 should be longest -> hypotenuse
          v31 = firstPoint - secondPoint;
          v32 = v32;
        } else if ((cv::norm(v32) > fp * cv::norm(v31)) &&
                   (cv::norm(v32) > fp * cv::norm(v12))) {
          v12 = v32;
          v32 = v31;
          v31 = firstPoint - secondPoint;
        }
        ...
        float projectedLength =
            std::abs((v32.x * v31.x + v32.y * v31.y)) / dist31 / dist32;
        float diffInLength = fabs(dist31 - dist32);

        // We are trying to maximise sumOfLength while minimizing
        // projectedLength and diffInLength
        if (best_score < (fw1 * sumOfLength -
                          (fw2 * projectedLength + fw3 * diffInLength))) {
          if (100.0 > dist12 && 80.0 > dist31 && 80.0 > dist32) {
            if (fabs(dist31 - dist32) < 0.5 * dist12) {
              ...
              best_score = fw1 * sumOfLength - fw2 * projectedLength -
                           fw3 * diffInLength;
            }
          }
        }
  ...
  if ((cv::norm(v12) > fp * cv::norm(v31)) &&
      (cv::norm(v12) > fp * cv::norm(v32))) {
    ;
  } else if ((cv::norm(v31) > fp * cv::norm(v32)) &&
             (cv::norm(v31) > fp * cv::norm(v12))) {
  ...
  point_list.erase(
      std::remove(point_list.begin(), point_list.end(), *cornerOne),
      point_list.end());
  point_list.erase(
      std::remove(point_list.begin(), point_list.end(), *cornerTwo),
      point_list.end());
  point_list.erase(
      std::remove(point_list.begin(), point_list.end(), *cornerThree),
      point_list.end());
...
LandmarkFinder::FindLandmarks(const std::vector<Cluster> &clusteredPoints) {
  ...
  for (auto &cluster : clusteredPoints) { /// go thru all clusters
    ...
    newLandmark.voIDPoints =
        cluster; /// all points in this cluster are copied to the ID point
                 /// vector for further examination
...
///--------------------------------------------------------------------------------------///
/// CalculateIdBackward searches in the filtered image for id points, given the
/// corners.
///
///--------------------------------------------------------------------------------------///
  ...
  for (int nX = 0; nX < 4; nX++) {
    for (int nY = 0; nY < 4; nY++) {
      ...
      if ((nX != 0 || nY != 0) && (nX != 0 || nY != 3) &&
        ...
        cv::Point Index(int(ThisPoint.at<float>(0, 0)),
                        int(ThisPoint.at<float>(1, 0)));
...
int LandmarkFinder::GetIDs(std::vector<ImgLandmark> &landmarks) {
  ...
  while (pLandmarkIt != landmarks.end()) {
    ...
    if (CalculateIdForward(*pLandmarkIt, validIDs)) {
      ...
      landmarksInQueue.push_back(*pLandmarkIt); /// put this landmark in queue
                                                /// for second processing run
      landmarks.erase(pLandmarkIt); /// delete it from valid landmark list. This
                                    /// also is a step to next landmark
    }
  }
...
void LandmarkFinder::parallel_vector_sort(std::vector<uint16_t> &ids,
                                          std::vector<cv::Point> &points) {
  size_t len = ids.size();
  ...
  while (b) {
    ...
    for (size_t i = 0; i < len; i++) {
      ...
      if (ids[i] >
          ids[i + stepsize]) // überprüft ob die zwei Elemente falsch herum sind
      {
