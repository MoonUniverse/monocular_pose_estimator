 ...
 * \brief File containing the definitions of the functions for the PoseEstimator
 * class
 *
 */
...
void PoseEstimator::augmentImage(cv::Mat &image) {
  Visualization::createVisualizationImage(
      image, predicted_pose_, camera_matrix_K_, camera_distortion_coeffs_,
...
List2DPoints PoseEstimator::getImagePoints() { return image_points_; }
...
VectorXuPairs PoseEstimator::getCorrespondences() { return correspondences_; }
...
double PoseEstimator::calculateSquaredReprojectionErrorAndCertainty(
    const List2DPoints &image_pts, const List2DPoints &object_pts,
    double &certainty) {
...
VectorXuPairs
PoseEstimator::correspondencesFromHistogram(MatrixXYu &histogram) {
...
unsigned PoseEstimator::checkCorrespondences() {
  ...
  if (correspondences_.rows() < 4) {
    ...
    MatrixXYu combinations =
        Combinations::combinationsNoReplacement(correspondences_.rows(), 3);
    ...
    for (unsigned i = 0; i < N; ++i) {
      ...
      temp_point =
          object_points_(correspondences_(combinations(i, 0) - 1, 0) - 1);
      world_points.col(0) = temp_point.head<3>();
      temp_point =
          object_points_(correspondences_(combinations(i, 1) - 1, 0) - 1);
      world_points.col(1) = temp_point.head<3>();
      temp_point =
          object_points_(correspondences_(combinations(i, 2) - 1, 0) - 1);
      world_points.col(2) = temp_point.head<3>();

      feature_vectors.col(0) =
          image_vectors_(correspondences_(combinations(i, 0) - 1, 1) - 1);
      feature_vectors.col(1) =
          image_vectors_(correspondences_(combinations(i, 1) - 1, 1) - 1);
      feature_vectors.col(2) =
          image_vectors_(correspondences_(combinations(i, 2) - 1, 1) - 1);
      ...
      List2DPoints unused_im_points(
          total_unused_correspondences); // Vector to hold the indexes of the
                                         // unused image points that have
                                         // correspondences
      List4DPoints unused_object_points(
          total_unused_correspondences); // Vector to hold the indexes of the
                                         // unused object points that have
                                         // correspondences
      ...
      // If the P3P algorithm found a solution (i.e. the world points were not
      // aligned), then continue
          ...
          if (isFinite(H_o_c_current)) {
            ...
            for (unsigned ii = 0; ii < total_unused_correspondences; ++ii) {
              unused_back_projected_object_points(ii) =
                  project2d(unused_object_points(ii), H_o_c_current.inverse());
            }

            double certainty;
            squared_error = calculateSquaredReprojectionErrorAndCertainty(
                unused_im_points, unused_back_projected_object_points,
                certainty);
    ...
    if ((double)num_valid_correspondences / N >=
      ...
      predicted_pose_ = computeTransformation(object_points_matrix,
                                              mean_reprojected_object_points);
    }
  }
  ...
  for (unsigned i = 0; i < num_seen_points_combinations; ++i) {
    ...
    for (unsigned j = 0; j < num_object_points_permutations; ++j) {
      ...
      if (executed_correctly == 0) {
        ...
        List4DPoints unused_object_points(
            total_unused_object_points); // Vector to hold the indexes of the
                                         // unused object points that have
                                         // correspondences
          ...
          if (isFinite(H_o_c_current)) {
            ...
            for (unsigned m = 0; m < total_unused_object_points; ++m) {
              unused_back_projected_object_points(m) =
                  project2d(unused_object_points(m), H_o_c_current.inverse());
            }
            ...
            if (count_within_pixel_threshold > 0) {
              ...
              for (unsigned mm = 0; mm < 3; ++mm) {
                im_idx =
                    seen_points_combinations(i, mm) - 1; // image point index
                obj_idx =
                    object_points_permutations(j, mm) - 1; // object point index
              ...
              for (unsigned nn = 0; nn < min_distances.size(); ++nn) {
                if (min_distances(nn) < back_projection_pixel_tolerance_) {
                  im_idx = unused_im_points_idx(pairs(nn, 0) -
                                                1); // image point index
                  obj_idx = unused_object_points_idx(pairs(nn, 1) -
...
void PoseEstimator::setPredictedTime(double time) { predicted_time_ = time; }

double PoseEstimator::getPredictedTime() { return predicted_time_; }
...
void PoseEstimator::optimisePose() {
  ...
  for (unsigned i = 0; i < max_itr; ++i) {
    ...
    for (unsigned j = 0; j < correspondences_.rows(); ++j) {
      ...
      Eigen::Vector2d p_image_plane =
          project2d((Eigen::Vector4d)object_points_(correspondences_(j, 0) - 1),
                    predicted_pose_);
...
void PoseEstimator::predictWithROI(double &time_to_predict,
                                   const cv::Mat &image) {
  if (it_since_initialized_ >=
      2) { // Predict the pose if initialised. If not the pose will remain the
           // same as the previous step (constant velocity model)
  ...
  region_of_interest_ = LEDDetector::determineROI(
      getPredictedPixelPositions(), image.size(), roi_border_thickness_,
      camera_matrix_K_, camera_distortion_coeffs_);
}
...
void PoseEstimator::findCorrespondencesAndPredictPose(double &time_to_predict) {
  ...
  if (checkCorrespondences() ==
    ...
    if (initialise() ==
        1) { // Only update pose if the initialisation found a valid pose.
      optimiseAndUpdatePose(time_to_predict);
    }
  }
}
...
Matrix2x6d
PoseEstimator::computeJacobian(const Eigen::Matrix4d &T_c_o,
  ...
  // thesis of Ethan Eade See http://ethaneade.com/ See
  // http://ethaneade.com/thesis_revised.pdf
...
Vector6d PoseEstimator::logarithmMap(const Eigen::Matrix4d &trans) {
  ...
  if (R.isApprox(Eigen::Matrix3d::Identity(), 1e-10) == 1) {
    ...
    // Force phi to be either 1 or -1 if necessary. Floating point errors can
    // cause problems resulting in this not happening
    if (temp > 1) {
      temp = 1;
    } else if (temp < -1) {
      temp = -1;
